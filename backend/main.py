...
async def scan_url(request: ScanRequest) -> ScanResponse:
    ...
    logger.info("Scanning URL: %s", original_url)

    # ── Level 0: Resolve shorteners / redirects ───────────────
    # Some domains (dub.sh, bit.ly, etc.) are "clean" by every
    # structural heuristic but only exist to redirect somewhere
    # else. Resolve to the final destination before analyzing,
    # otherwise the pipeline scores the wrapper, not the target.
    final_url, redirect_chain, was_shortener = await resolve_final_url(original_url)
    if final_url != original_url:
        logger.info(
            "URL resolved: %s -> %s (%d hop(s), shortener=%s)",
            original_url, final_url, len(redirect_chain) - 1, was_shortener,
        )
    url = final_url
    ...
    # ── Surface the redirect chain so the user sees the masking ──
    # response.url shows the ORIGINAL url the user submitted;
    # the score itself is computed on the resolved destination.
    response.url = original_url
    if final_url != original_url:
        hop_note = (
            f"🔀 Ссылка ведёт через {len(redirect_chain) - 1} редирект(ов) "
            f"на: {final_url}"
        )
        if was_shortener:
            hop_note += " (обнаружен известный сокращатель ссылок)"
        response.reasons.insert(0, hop_note)
